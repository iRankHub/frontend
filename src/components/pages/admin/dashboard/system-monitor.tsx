  ...
  return (
    <div className="mb-4">
      {status.issues.length > 0 && (
        <Alert variant={status.status === 'critical' ? 'destructive' : 'warning'}>
          <Icon className="h-4 w-4" />
          <AlertTitle>
            {status.status === 'critical' ? 'System Critical' : 'System Warning'}
          </AlertTitle>
          <AlertDescription>
...
const HealthMetric: React.FC<HealthMetricProps> = ({ 
  ...
  return (
    <div className={`w-full h-auto flex rounded-md overflow-hidden border ${
        ...
        <div className="block w-64">
          <div className="flex justify-between">
            ...
            <p className="text-sm text-muted-foreground">
              {usedValue}{unit} / {totalValue}{unit}
            </p>
          </div>
          <Progress 
            value={percentage} 
...
const SystemMonitor: React.FC = () => {
  ...
  const getSystemHealth = async (): Promise<void> => {
  ...
  useEffect(() => {
    getSystemHealth();
    const interval = setInterval(getSystemHealth, 30000);
    return () => clearInterval(interval);
  }, []);
  ...
  const getMetricStatus = (percentage: number): 'healthy' | 'warning' | 'critical' => {
    ...
    <div className="col-span-2 px-7 py-5 bg-background rounded-lg border-2 border-muted mt-5">
      <div className="flex justify-between items-center mb-4">
        ...
        {!isLoading && systemHealth && (
          <div className={`px-3 py-1 rounded-full text-sm ${
            systemStatus.status === 'critical' ? 'bg-destructive/10 text-destructive' :
            systemStatus.status === 'warning' ? 'bg-warning/10 text-warning' :
            'bg-success/10 text-success'
          }`}>
            {systemStatus.status === 'critical' ? 'System Critical' :
             systemStatus.status === 'warning' ? 'System Warning' :
      ...
      {isLoading && (
        <div className="w-full mt-4">
          <p className="text-sm text-muted-foreground mb-2">Loading system health data...</p>
        ...
        {systemHealth && (
          <>
            ...
            <div className="flex gap-4 mt-2">
              {[
                { label: "Node Count", value: systemHealth.nodeCount },
                { label: "Pod Count", value: systemHealth.podCount },
                { label: "PVC Count", value: systemHealth.pvcCount }
              ].map((metric) => (
                <div key={metric.label} className="flex-1 p-4 border border-muted rounded-md">
