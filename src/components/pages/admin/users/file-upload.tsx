...
function FileUpload({}: Props) {
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    try {
      ...
      const res = await batchCreateUsers(parsedData);
  ...
  const handleRedirectToTemplateSheet = () => {
    window.open("https://docs.google.com/spreadsheets/d/1Dy2bnTc1PkQBsdf-rugz0JpmGufTsQW8F7M0rezZXa0/edit?gid=1130316211#gid=1130316211", "_blank");
  };
    ...
    <Card className="p-3">
      <div className="flex items-start justify-between gap-5">
        <div className="flex-1 flex items-start gap-3">
          <Icons.fileUp className={`w-5 h-5 ${variant === "failed" ? "text-destructive" : "text-[#0F172A]"}`} />
          <div className="w-full flex flex-col p-0 m-0">
            <span className={`text-sm font-medium m-0 p-0 leading-5 ${variant === "failed" ? "text-destructive" : ""}`}>
              Users.xlsx
            </span>
            <small className="text-muted-foreground uppercase w-full">{size}</small>
            {cardContent[variant]}
          </div>
        </div>
        {variant === "success" ? (
          ...
          <Trash2 className={`w-5 h-5 ${variant === "failed" ? "text-destructive" : "text-foreground"}`} />
        )}
      </div>
    </Card>
  );
