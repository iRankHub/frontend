...
function Reports() {
  ...
  const [selectedTournament, setSelectedTournament] = useState<TournamentSearchResult.AsObject | null>(null);
  const [isSearching, setIsSearching] = useState(false);
  const [tournaments, setTournaments] = useState<TournamentSearchResult.AsObject[]>([]);
  ...
  const handleTournamentSelect = (tournament: TournamentSearchResult.AsObject) => {
    ...
    <div className="w-full mt-5 rounded-md bg-background pb-10">
      ...
      <Tabs defaultValue="financial" className="mt-4 mx-auto w-full">
        <TabsList className="max-w-96 mx-5">
          <TabsTrigger value="financial" className="data-[state=active]:text-white">
            Financial
          </TabsTrigger>
          <TabsTrigger value="attendance" className="data-[state=active]:text-white">
            Attendance
          </TabsTrigger>
          <TabsTrigger value="performance" className="data-[state=active]:text-white">
            Performance
          </TabsTrigger>
        </TabsList>
        <TabsContent value="financial" className="w-full h-full">
          <FinancialReports tournamentId={selectedTournament?.tournamentId} />
        </TabsContent>
