...
const LoginFormEmail: React.FC<LoginFormEmailProps> = ({ handleChange }) => {
  ...
  async function onSubmit(data: Inputs) {
    setIsPending(true);
    await schoolLogin({ emailOrId: data.email, password: data.password })
      .then((res) => {
        if (res.success) {
          toast({
            variant: "success",
            title: "Success",
            description: "The schools Dashboard is currently going throught maintenance. Please check back later.",
            action: (
              <ToastAction
                altText="Close"
